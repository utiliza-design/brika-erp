  ...
  if (editing) {
    return (
      <input type="date" autoFocus defaultValue={toInputDate(fechaCobro)}
        ...
        data-testid={`input-fecha-cobro-g66clp-${movementKey}`}
      />
    );
  }
  ...
  return (
    <div className="flex items-center gap-1 group">
      <button onClick={() => setEditing(true)}
        ...
        data-testid={`text-fecha-cobro-g66clp-${movementKey}`}
      >{fechaCobro}</button>
      {isModified && (
        <button onClick={() => onSave(movementKey, null)}
          className="opacity-0 group-hover:opacity-60 hover:opacity-100 transition-opacity"
          title="Restaurar a fecha de emisión"
          data-testid={`btn-reset-fecha-cobro-g66clp-${movementKey}`}
        >
...
function NDocCell({ movementKey, nDocumentos, nDocIsOverride, candidatos, allRows, onSave }: {
  ...
  candidatos: Candidato[]; allRows: G66ClpRow[];
  ...
  return (
    <Popover open={open} onOpenChange={(o) => { setOpen(o); if (!o) { setPendingValue(null); setWarnings([]); setSearch(""); } }}>
      <PopoverTrigger asChild>
        <button className="flex items-center flex-wrap gap-1 group focus:outline-none" data-testid={`btn-ndoc-g66clp-${movementKey}`}>
                    ...
                    <TooltipTrigger asChild>
                      <span>
                        <Badge variant={nDocIsOverride ? "secondary" : "outline"}
                          ...
                          data-testid={`badge-ndoc-g66clp-${doc}`}
                        >
                          #{doc}
                          {nDocIsOverride && (
        ...
        {warnings.length > 0 ? (
          <div className="p-3 space-y-3">
            ...
            <div className="flex gap-2">
              <Button size="sm" className="h-7 text-xs flex-1" onClick={() => confirmSave(pendingValue!)} data-testid="btn-confirm-ndoc-g66clp">Confirmar</Button>
              <Button size="sm" variant="outline" className="h-7 text-xs flex-1" onClick={cancelPending} data-testid="btn-cancel-ndoc-g66clp">Cancelar</Button>
            </div>
          </div>
        ) : (
          <>
            ...
            <div className="p-2 border-b">
              <Input placeholder="Buscar N° documento o cliente..." value={search} onChange={(e) => setSearch(e.target.value)}
                className="h-8 text-xs" autoFocus data-testid="input-search-ndoc-g66clp"
              />
            </div>
                  ...
                  filtered.map((c) => (
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        ...
                        {c.monto > 0 && <p className="text-[11px] text-muted-foreground leading-tight">${Math.round(c.monto).toLocaleString("es-CL")}{c.fecha ? ` · ${c.fecha}` : ""}</p>}
                      </div>
                    </button>
                  ))
                )}
            ...
            <div className="border-t p-2 space-y-2">
              ...
              <div className="flex gap-1.5">
                <Input value={manualValue} onChange={(e) => setManualValue(e.target.value)} placeholder="Ej: 12345"
                  className="h-7 text-xs" onKeyDown={(e) => e.key === "Enter" && attemptSave(manualValue.trim())} data-testid="input-manual-ndoc-g66clp"
                />
                <Button size="sm" className="h-7 text-xs px-2" onClick={() => attemptSave(manualValue.trim())} disabled={!manualValue.trim()} data-testid="btn-save-ndoc-g66clp">Guardar</Button>
              </div>
              {nDocIsOverride && (
                <Button variant="ghost" size="sm" className="h-7 text-xs text-muted-foreground w-full hover:text-destructive" onClick={handleClearAll} data-testid="btn-clear-ndoc-g66clp">
...
const API_PREFIX = "/api/global66-clp-costos";

export default function CajaGlobal66ClpPage() {
  ...
  const { data: rows, isLoading } = useQuery<G66ClpRow[]>({ queryKey: [API_PREFIX] });
  ...
  const handleToggleAll = async (filteredRowsSnapshot: G66ClpRow[]) => {
    ...
    <div className="px-6 py-6 pb-8" style={{ width: "max-content", minWidth: "100%" }}>
      <div className="max-w-6xl space-y-6 mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight" data-testid="text-g66clp-costos-title">
              Caja - Global66 CLP
            </h1>
            <p className="text-muted-foreground text-sm mt-0.5">
              Movimientos de la cuenta Global66 en pesos chilenos
            </p>
          </div>
          {rows && rows.length > 0 && (
            <div className="ml-auto flex items-center gap-3">
              <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200 dark:bg-green-950 dark:text-green-300 dark:border-green-800" data-testid="badge-g66clp-revisados">
                <Check className="h-3 w-3 mr-1" />{statsResumen.revisados} revisados
              </Badge>
              <Badge variant="outline" className="bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-950 dark:text-amber-300 dark:border-amber-800" data-testid="badge-g66clp-pendientes">
                {statsResumen.pendientes} pendientes
              </Badge>
              {filteredRows.length > 0 && (
                <Button variant="outline" size="sm" className="gap-2" data-testid="button-download-g66clp"
                  onClick={() => {
                    const COLS = [
                    ...
                    downloadAsXlsx(data, COLS, "caja-global66-clp");
                  }}
                >
            ...
            <div className="flex flex-wrap gap-4 items-end">
              ...
              <div className="space-y-1.5">
                ...
                <Select value={filtroCentro} onValueChange={setFiltroCentro}>
                  <SelectTrigger className="w-56" data-testid="select-g66clp-filtro-centro"><SelectValue placeholder="Todos" /></SelectTrigger>
                  <SelectContent>
              ...
              <div className="space-y-1.5">
                ...
                <Select value={filtroEstado} onValueChange={setFiltroEstado}>
                  <SelectTrigger className="w-44" data-testid="select-g66clp-filtro-estado"><SelectValue placeholder="Todos" /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="todos">Todos</SelectItem>
              ...
              <div className="space-y-1.5">
                ...
                <Select value={filtroTipo} onValueChange={setFiltroTipo}>
                  <SelectTrigger className="w-44" data-testid="select-g66clp-filtro-tipo"><SelectValue placeholder="Todos" /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="todos">Todos</SelectItem>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
            <Card className="border-card-border">
              <CardContent className="pt-5 pb-4">
                ...
                <p className={`text-2xl font-bold ${(rows && rows.length > 0 ? (rows[0].saldoBanco ?? 0) : 0) >= 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`} data-testid="text-g66clp-saldo-actual">
            ...
            <Card className="border-card-border">
              <CardContent className="pt-5 pb-4">
                ...
                <p className="text-2xl font-bold text-green-600 dark:text-green-400" data-testid="text-g66clp-total-abonos">
            ...
            <Card className="border-card-border">
              <CardContent className="pt-5 pb-4">
                ...
                <p className="text-2xl font-bold text-red-600 dark:text-red-400" data-testid="text-g66clp-total-debitos">
            ...
            <Card className="border-card-border">
              <CardContent className="pt-5 pb-4">
                ...
                <p className="text-2xl font-bold text-orange-600 dark:text-orange-400" data-testid="text-g66clp-total-sin-nada">
      ...
      <Card className="border-card-border w-full">
        <CardHeader className="flex flex-row items-center justify-between gap-2 space-y-0 pb-4">
          <CardTitle className="text-base font-semibold" data-testid="text-g66clp-tabla-movimientos">Movimientos</CardTitle>
          {filteredRows.length > 0 && (
            <div className="flex items-center gap-4">
              <div className="text-right">
                <p className="text-xs text-muted-foreground">Saldo</p>
                <p className={`text-lg font-bold tabular-nums ${(rows && rows.length > 0 ? (rows[0].saldoBanco ?? 0) : 0) < 0 ? "text-red-600 dark:text-red-400" : "text-green-700 dark:text-green-400"}`} data-testid="text-g66clp-total-visible">
                  {formatMonto(rows && rows.length > 0 ? (rows[0].saldoBanco ?? 0) : 0)}
                </p>
              </div>
              <span className="text-xs text-muted-foreground border-l pl-4" data-testid="text-g66clp-row-count">
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-xs text-muted-foreground/70 mt-1">Sube un archivo PDF de Global66 CLP en el módulo de Subir Archivos</p>
            </div>
          ) : filteredRows.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <Table>
                {(() => {
                  ...
                  const SortBtn = ({ field, label, className }: { field: string; label: string; className?: string }) => (
                    <TableHead className={`${thBase} ${className ?? ""}`}>
                      <button className="flex items-center gap-0.5 hover:text-foreground transition-colors" onClick={() => toggleSort(field)} data-testid={`sort-g66clp-${field}`}>
                  ...
                  return (
                    <TableHeader>
                      <TableRow>
                        ...
                        <SortBtn field="debito" label="Débito" className="text-right w-24" />
                        <SortBtn field="abono" label="Abono" className="text-right w-24" />
                        ...
                        <TableHead className={`${thBase} w-14 text-center !pr-8`}>
                          <Checkbox
                            ...
                            data-testid="checkbox-g66clp-review-all"
                ...
                <TableBody>
                  {sortedRows.map((row, i) => (
                    <TableRow key={i} data-testid={`row-g66clp-movimiento-${i}`} className={row.revisado ? "bg-green-50/50 dark:bg-green-950/20" : ""}>
                      <TableCell className="text-sm" data-testid={`text-g66clp-fecha-${i}`}>{row.fecha}</TableCell>
                      <TableCell data-testid={`cell-g66clp-fecha-cobro-${i}`}>
                        <FechaCobroCell movementKey={row.movementKey} fechaCobro={row.fechaCobro} fechaEmision={row.fecha} onSave={handleSaveFechaCobro} />
                      </TableCell>
                      <TableCell className="text-sm max-w-[200px]" data-testid={`text-g66clp-detalle-${i}`}>
                        <div className="truncate" title={row.detalle}>{row.detalle}</div>
                      </TableCell>
                      <TableCell className="text-xs text-muted-foreground font-mono" data-testid={`text-g66clp-movimiento-${i}`}>{row.movimientoNro || "—"}</TableCell>
                      <TableCell data-testid={`text-g66clp-ndocumento-${i}`}>
                      ...
                      <TableCell className="text-sm font-medium text-right text-red-600 dark:text-red-400 tabular-nums" data-testid={`text-g66clp-debito-${i}`}>
                        {row.debito > 0 ? formatMonto(-row.debito) : "—"}
                      </TableCell>
                      <TableCell className="text-sm font-medium text-right text-green-600 dark:text-green-400 tabular-nums" data-testid={`text-g66clp-abono-${i}`}>
                        {row.abono > 0 ? formatMonto(row.abono) : "—"}
                      </TableCell>
                      <TableCell className={`text-sm font-semibold text-right ${(row.saldoBanco ?? 0) < 0 ? "text-red-600 dark:text-red-400" : "text-foreground"}`} data-testid={`text-g66clp-saldo-${i}`}>
                        {row.saldoBanco != null ? formatMonto(row.saldoBanco) : "—"}
                      </TableCell>
                      <TableCell data-testid={`cell-g66clp-centro-costos-${i}`}>
                        {row.centroCostos ? (
                          <div className="flex items-center gap-2">
                            ...
                            <Select value={row.centroCostos} onValueChange={(val) => handleCentroChange(row.detalle, val, row.movementKey)}>
                              <SelectTrigger className="h-7 w-7 p-0 border-none shadow-none [&>svg]:h-3 [&>svg]:w-3" data-testid={`btn-g66clp-change-centro-${i}`}>
                          ...
                          <Select value="" onValueChange={(val) => handleCentroChange(row.detalle, val, row.movementKey)}>
                            <SelectTrigger className="h-7 w-36 text-xs text-muted-foreground" data-testid={`select-g66clp-centro-${i}`}>
                      ...
                      <TableCell className="text-center" data-testid={`cell-g66clp-estado-${i}`}>
                        {row.revisado ? (
