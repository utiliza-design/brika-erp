...
function extractPattern(detalle: string): string {
  let text = detalle.trim();
  const colonIdx = text.indexOf(":");
  if (colonIdx !== -1) {
    text = text.substring(colonIdx + 1).trim();
  }
  text = text.replace(/\b\d{5,}\b/g, "").trim();
  text = text.replace(/\b\d{2}\/\d{2}\/\d{4}\b/g, "").trim();
  text = text.replace(/\b\d{4}-\d{2}-\d{2}\b/g, "").trim();
  text = text.replace(/\s{2,}/g, " ").trim();
  text = text.replace(/[-_/\\]+$/, "").trim();
  if (text.length > 40) text = text.substring(0, 40).trim();
  return text || detalle.trim();
}
...
function NDocCell({
  ...
  const handleSelect = (nDoc: string) => {
    attemptSave(nDoc);
  };
    ...
    <Popover open={open} onOpenChange={(o) => { setOpen(o); if (!o) { setPendingValue(null); setWarnings([]); setSearch(""); } }}>
      <PopoverTrigger asChild>
        <button
          ...
          data-testid={`btn-ndoc-${movementKey}`}
        >
                        ...
                        >
                          ...
                          {nDocIsOverride && (
                            <span
                              ...
                              data-testid={`btn-remove-ndoc-${doc}`}
                            >
      ...
      <PopoverContent className="w-80 p-0" align="start" data-testid={`popover-ndoc-${movementKey}`}>
        {warnings.length > 0 ? (
          <div className="p-3 space-y-3" data-testid="ndoc-warning-panel">
            ...
            <div className="flex gap-2">
              ...
              <Button
                ...
                data-testid="btn-cancel-ndoc-warning"
              >
                Cancelar
              </Button>
            </div>
                ...
                {nDocumentos.map((doc) => (
                  <Badge key={doc} variant="secondary" className="text-xs font-mono bg-primary/10 text-primary border-0 pr-1">
                    ...
                    {nDocIsOverride && (
                      <span
                        role="button"
                        onClick={(e) => { handleRemoveOne(doc, e); setOpen(false); }}
                        className="ml-1 opacity-50 hover:opacity-100 transition-opacity"
                      >
                        <X className="h-2.5 w-2.5 inline" />
                      </span>
                ...
                {filtered.length === 0 ? (
                  ...
                  filtered.map((c) => (
                    <button
                      ...
                      onClick={() => handleSelect(c.nDocumento)}
                      ...
                      data-testid={`candidato-ndoc-${c.nDocumento}`}
                    >
                      ...
                      <div className="min-w-0">
                        ...
                        {c.descripcion && (
                          <p className="text-[11px] text-muted-foreground truncate leading-tight">{c.descripcion}</p>
                        )}
                        {c.monto > 0 && (
            ...
            <div className="border-t p-2 space-y-2">
              ...
              {nDocIsOverride && (
                <Button
                  variant="ghost"
                  size="sm"
                  className="h-7 text-xs text-muted-foreground w-full hover:text-destructive"
                  onClick={handleClearAll}
                  data-testid="btn-clear-ndoc"
                >
  ...
  if (editing) {
    return (
      <input
        ...
        data-testid={`input-fecha-cobro-${movementKey}`}
      />
    );
  }
  ...
  return (
    <div className="flex items-center gap-1 group">
      <button
        ...
        data-testid={`text-fecha-cobro-${movementKey}`}
      >
        {fechaCobro}
      </button>
      {isModified && (
        <button
          ...
          data-testid={`btn-reset-fecha-cobro-${movementKey}`}
        >
...
export default function CentroCostosPage() {
  ...
  const { data: rows, isLoading } = useQuery<CentroCostosRow[]>({
    queryKey: ["/api/centro-costos"],
  });

  const { data: candidatos = [] } = useQuery<Candidato[]>({
    queryKey: ["/api/centro-costos/candidatos-ndoc"],
    staleTime: 5 * 60 * 1000,
  });

  const saveNDocMutation = useMutation({
    mutationFn: async ({ movementKey, nDocumentos }: { movementKey: string; nDocumentos: string[] }) => {
      const res = await apiRequest("POST", "/api/centro-costos/save-ndocumento", { movementKey, nDocumentos });
      return res.json();
    },
    onMutate: async ({ movementKey, nDocumentos }) => {
      await queryClient.cancelQueries({ queryKey: ["/api/centro-costos"] });
      const snapshot = queryClient.getQueryData<CentroCostosRow[]>(["/api/centro-costos"]);
      queryClient.setQueryData<CentroCostosRow[]>(["/api/centro-costos"], (old) =>
        old ? old.map(r => r.movementKey === movementKey ? { ...r, nDocumentos, nDocIsOverride: true } : r) : old
      );
      return { snapshot };
    },
    onError: (_err: unknown, _vars: unknown, context: { snapshot?: CentroCostosRow[] } | undefined) => {
      if (context?.snapshot) queryClient.setQueryData(["/api/centro-costos"], context.snapshot);
      toast({ title: "Error", description: "No se pudo guardar el N° de documento", variant: "destructive" });
    },
    onSettled: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/centro-costos"] });
    },
  });
  ...
  const saveFechaCobroMutation = useMutation({
    mutationFn: async ({ movementKey, fechaCobro }: { movementKey: string; fechaCobro: string | null }) => {
      const res = await apiRequest("POST", "/api/centro-costos/save-fecha-cobro", { movementKey, fechaCobro });
      return res.json();
    },
  ...
  const saveCCMutation = useMutation({
    mutationFn: async ({ movementKey, centroCostos }: { movementKey: string; centroCostos: string }) => {
      const res = await apiRequest("POST", "/api/centro-costos/save-cc", { movementKey, centroCostos });
      return res.json();
    },
  ...
  const reviewMutation = useMutation({
    mutationFn: async ({ movementKey, revisado }: { movementKey: string; revisado: boolean }) => {
      const res = await apiRequest("POST", "/api/centro-costos/review", { movementKey, revisado });
      return res.json();
    },
  ...
  const handleToggleAll = async (filteredRowsSnapshot: CentroCostosRow[]) => {
    const allDone = filteredRowsSnapshot.length > 0 && filteredRowsSnapshot.every(r => r.revisado);
    const targetState = !allDone;
    setIsMarkingAll(true);
    await queryClient.cancelQueries({ queryKey: ["/api/centro-costos"] });
    const snapshot = queryClient.getQueryData<CentroCostosRow[]>(["/api/centro-costos"]);
    const affectedKeys = new Set(filteredRowsSnapshot.filter(r => r.revisado !== targetState).map(r => r.movementKey));
    queryClient.setQueryData<CentroCostosRow[]>(["/api/centro-costos"], (old) =>
      old ? old.map(r => affectedKeys.has(r.movementKey) ? { ...r, revisado: targetState } : r) : old
    );
    try {
      await Promise.all(
        filteredRowsSnapshot
          .filter(r => r.revisado !== targetState)
          .map(r => apiRequest("POST", "/api/centro-costos/review", { movementKey: r.movementKey, revisado: targetState }).then(res => res.json()))
      );
      toast({ title: "Actualizado", description: `Movimientos marcados como ${targetState ? "revisados" : "pendientes"}` });
    } catch {
      if (snapshot) queryClient.setQueryData(["/api/centro-costos"], snapshot);
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) => {
      ...
      if (filtroCentro !== "todos") {
        if (filtroCentro === "sin-asignar") {
          if (row.centroCostos) return false;
        } else {
          if (row.centroCostos !== filtroCentro) return false;
        }
      }
  ...
  const centrosUsados = useMemo(() => {
    ...
    for (const row of rows) {
      if (row.centroCostos) set.add(row.centroCostos);
    }
  ...
  const totales = useMemo(() => {
    ...
    for (const row of filteredRows) {
      if (row.monto >= 0) ventas += row.monto;
        ...
        costos += row.monto;
        if (row.centroCostos !== "C-NADA") costossinNada += row.monto;
      }
    }
  ...
  return (
    <div className="px-6 py-6 pb-8" style={{ width: "max-content", minWidth: "100%" }}>
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight" data-testid="text-centro-costos-title">
            Caja - Banco de Chile
          </h1>
          <p className="text-muted-foreground text-sm mt-0.5">
            Movimientos bancarios de la Cartola Banco de Chile
          </p>
        </div>
        {rows && rows.length > 0 && (
          <div className="ml-auto flex items-center gap-3">
            <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200 dark:bg-green-950 dark:text-green-300 dark:border-green-800" data-testid="badge-revisados">
              <Check className="h-3 w-3 mr-1" />
              {statsResumen.revisados} revisados
            </Badge>
            <Badge variant="outline" className="bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-950 dark:text-amber-300 dark:border-amber-800" data-testid="badge-pendientes">
            ...
            {filteredRows.length > 0 && (
              <Button
                ...
                size="sm"
                className="gap-2"
                data-testid="button-download-caja"
                onClick={() => {
                  const DOWNLOAD_COLS = [
                    { key: "fecha",            label: "Fecha Emisión" },
                    { key: "fechaCobro",       label: "Fecha Cobro" },
                    { key: "sucursal",         label: "Banco / Sucursal" },
      ...
      {user?.email === "natyjelen@gmail.com" && (
      ...
      <Card className="border-card-border">
        ...
        <CardContent>
          <div className="flex flex-wrap gap-4 items-end">
            ...
            <div className="space-y-1.5">
              <Label htmlFor="filtro-centro" className="text-sm">Centro de Costos</Label>
              <Select value={filtroCentro} onValueChange={setFiltroCentro}>
                <SelectTrigger className="w-56" data-testid="select-filtro-centro">
            ...
            <div className="space-y-1.5">
              <Label htmlFor="filtro-estado" className="text-sm">Estado</Label>
              <Select value={filtroEstado} onValueChange={setFiltroEstado}>
                <SelectTrigger className="w-44" data-testid="select-filtro-estado">
                  <SelectValue placeholder="Todos" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="todos">Todos</SelectItem>
            ...
            <div className="space-y-1.5">
              <Label htmlFor="filtro-tipo" className="text-sm">Tipo transacción</Label>
              <Select value={filtroTipo} onValueChange={setFiltroTipo}>
                <SelectTrigger className="w-44" data-testid="select-filtro-tipo">
                  <SelectValue placeholder="Todos" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="todos">Todos</SelectItem>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
          <Card className="border-card-border">
            <CardContent className="pt-5 pb-4">
              ...
              <p
                className={`text-2xl font-bold ${(rows && rows.length > 0 ? (rows[0].saldoBanco ?? 0) : 0) >= 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}
                data-testid="text-saldo-actual"
              >
          ...
          <Card className="border-card-border">
            <CardContent className="pt-5 pb-4">
              <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-1">Total Venta</p>
              <p className="text-2xl font-bold text-green-600 dark:text-green-400" data-testid="text-total-ventas">
          ...
          <Card className="border-card-border">
            <CardContent className="pt-5 pb-4">
              <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-1">Total Costos</p>
              <p className="text-2xl font-bold text-red-600 dark:text-red-400" data-testid="text-total-costos">
          ...
          <Card className="border-card-border">
            <CardContent className="pt-5 pb-4">
              <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-1">Total Costos Descontando "C-Nada"</p>
              <p className="text-2xl font-bold text-orange-600 dark:text-orange-400" data-testid="text-total-costos-sin-nada">
                {totales.costossinNada.toLocaleString("es-CL", { style: "currency", currency: "CLP", minimumFractionDigits: 0 })}
              </p>
              <p className="text-xs text-muted-foreground mt-1">Costos excluyendo C-NADA</p>
            </CardContent>
          </Card>
        </div>
      )}
      ...
      <Card className="border-card-border w-full">
        <CardHeader className="flex flex-row items-center justify-between gap-2 space-y-0 pb-4">
          <CardTitle className="text-base font-semibold" data-testid="text-tabla-movimientos">
            Movimientos
          </CardTitle>
          {filteredRows.length > 0 && (
            <div className="flex items-center gap-4">
              <div className="text-right">
                ...
                <p
                  className={`text-lg font-bold tabular-nums ${(rows && rows.length > 0 ? (rows[0].saldoBanco ?? 0) : 0) < 0 ? "text-red-600 dark:text-red-400" : "text-green-700 dark:text-green-400"}`}
                  data-testid="text-total-visible"
                >
                  {formatMonto(rows && rows.length > 0 ? (rows[0].saldoBanco ?? 0) : 0)}
                </p>
              </div>
              <span className="text-xs text-muted-foreground border-l pl-4" data-testid="text-row-count">
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-xs text-muted-foreground/70 mt-1">
                Sube un archivo de Cartola Banco en el módulo correspondiente
              </p>
            </div>
          ) : filteredRows.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <Table>
                  ...
                  return (
                    <TableHeader>
                      <TableRow>
                        ...
                        <SortBtn field="detalle" label="Detalle Movimiento" />
                        ...
                        <SortBtn field="sucursal" label="Sucursal" className="w-16" />
                ...
                <TableBody>
                  {sortedRows.map((row, i) => (
                    ...
                    >
                      <TableCell className="text-sm" data-testid={`text-fecha-${i}`}>
                        {row.fecha}
                      </TableCell>
                      <TableCell data-testid={`cell-fecha-cobro-${i}`}>
                        <FechaCobroCell
                      ...
                      <TableCell className="text-sm max-w-[200px]" data-testid={`text-detalle-${i}`}>
                        <div className="truncate" title={row.detalle}>{row.detalle}</div>
                      </TableCell>
                      <TableCell data-testid={`text-ndocumento-${i}`}>
                        <NDocCell
                          movementKey={row.movementKey}
                      ...
                      <TableCell
                        className={`text-sm font-medium text-right ${
                          row.monto < 0 ? "text-red-600 dark:text-red-400" : "text-green-600 dark:text-green-400"
                        }`}
                        data-testid={`text-monto-${i}`}
                      >
                        {formatMonto(row.monto)}
                      </TableCell>
                      <TableCell
                        className={`text-sm font-semibold text-right ${
                          (row.saldoBanco ?? 0) < 0 ? "text-red-600 dark:text-red-400" : "text-foreground"
                        }`}
                        data-testid={`text-saldo-${i}`}
                      >
                        {row.saldoBanco != null ? formatMonto(row.saldoBanco) : "—"}
                      </TableCell>
                      <TableCell className="text-sm text-muted-foreground" data-testid={`text-sucursal-${i}`}>
                        {row.sucursal}
                      </TableCell>
                      <TableCell data-testid={`cell-centro-costos-${i}`}>
                        {row.centroCostos ? (
                          <div className="flex items-center gap-2">
                            ...
                            <Select
                              ...
                              onValueChange={(val) => handleCentroChange(row.detalle, val, row.movementKey)}
                            >
                              <SelectTrigger className="h-7 w-7 p-0 border-none shadow-none [&>svg]:h-3 [&>svg]:w-3" data-testid={`btn-change-centro-${i}`}>
                                <span className="sr-only">Cambiar</span>
                              </SelectTrigger>
                              <SelectContent>
                                {CENTROS_DE_COSTOS.map((c) => (
                                  <SelectItem key={c} value={c}>{c}</SelectItem>
                                ))}
                              </SelectContent>
                            </Select>
                          ...
                          <Select
                            ...
                            onValueChange={(val) => handleCentroChange(row.detalle, val, row.movementKey)}
                          >
                            <SelectTrigger className="h-7 w-36 text-xs text-muted-foreground" data-testid={`select-centro-${i}`}>
                      ...
                      <TableCell className="text-center" data-testid={`cell-estado-${i}`}>
                        {row.revisado ? (
