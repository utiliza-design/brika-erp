    ...
    <Popover open={open} onOpenChange={(o) => { setOpen(o); if (!o) { setPendingValue(null); setWarnings([]); setSearch(""); } }}>
      <PopoverTrigger asChild>
        <button
          ...
          data-testid={`btn-ndoc-sec-${movementKey}`}
        >
                    ...
                    <TooltipTrigger asChild>
                      <span>
                        <Badge
                          ...
                          data-testid={`badge-ndoc-sec-${doc}`}
                        >
                          #{doc}
                          {nDocIsOverride && (
                            <span
        ...
        {warnings.length > 0 ? (
          <div className="p-3 space-y-3">
            ...
            <div className="flex gap-2">
              <Button size="sm" className="h-7 text-xs flex-1" onClick={() => confirmSave(pendingValue!)} data-testid="btn-confirm-ndoc-sec">Confirmar</Button>
              <Button size="sm" variant="outline" className="h-7 text-xs flex-1" onClick={cancelPending} data-testid="btn-cancel-ndoc-sec">Cancelar</Button>
            </div>
          </div>
        ) : (
          <>
            ...
            <div className="p-2 border-b">
              <Input
                ...
                autoFocus
                data-testid="input-search-ndoc-sec"
              />
            </div>
            ...
            <div className="border-t p-2 space-y-2">
              ...
              <div className="flex gap-1.5">
                <Input value={manualValue} onChange={(e) => setManualValue(e.target.value)} placeholder="Ej: 12345" className="h-7 text-xs" onKeyDown={(e) => e.key === "Enter" && attemptSave(manualValue.trim())} data-testid="input-manual-ndoc-sec" />
                <Button size="sm" className="h-7 text-xs px-2" onClick={() => attemptSave(manualValue.trim())} disabled={!manualValue.trim()} data-testid="btn-save-ndoc-sec">Guardar</Button>
              </div>
              {nDocIsOverride && (
                <Button variant="ghost" size="sm" className="h-7 text-xs text-muted-foreground w-full hover:text-destructive" onClick={handleClearAll} data-testid="btn-clear-ndoc-sec">
  ...
  if (editing) {
    return (
      <input
        ...
        data-testid={`input-fecha-cobro-sec-${movementKey}`}
      />
    );
  }
  ...
  return (
    <div className="flex items-center gap-1 group">
      <button
        ...
        data-testid={`text-fecha-cobro-sec-${movementKey}`}
      >
        {fechaCobro}
      </button>
      {isModified && (
        <button
          ...
          data-testid={`btn-reset-fecha-cobro-sec-${movementKey}`}
        >
...
export default function CajaSecurityPage() {
  const { user } = useAuth();
  ...
  const { data: rows, isLoading } = useQuery<SecurityRow[]>({
    queryKey: ["/api/security-costos"],
  });

  const { data: candidatos = [] } = useQuery<Candidato[]>({
    queryKey: ["/api/security-costos/candidatos-ndoc"],
    staleTime: 5 * 60 * 1000,
  });

  const saveNDocMutation = useMutation({
    mutationFn: async ({ movementKey, nDocumentos }: { movementKey: string; nDocumentos: string[] }) => {
      const res = await apiRequest("POST", "/api/security-costos/save-ndocumento", { movementKey, nDocumentos });
      return res.json();
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["/api/security-costos"] }); },
  ...
  const saveFechaCobroMutation = useMutation({
    mutationFn: async ({ movementKey, fechaCobro }: { movementKey: string; fechaCobro: string | null }) => {
      const res = await apiRequest("POST", "/api/security-costos/save-fecha-cobro", { movementKey, fechaCobro });
      return res.json();
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["/api/security-costos"] }); },
  ...
  const saveCCMutation = useMutation({
    mutationFn: async ({ movementKey, centroCostos }: { movementKey: string; centroCostos: string }) => {
      const res = await apiRequest("POST", "/api/security-costos/save-cc", { movementKey, centroCostos });
      return res.json();
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["/api/security-costos"] }); },
  ...
  const reviewMutation = useMutation({
    mutationFn: async ({ movementKey, revisado }: { movementKey: string; revisado: boolean }) => {
      const res = await apiRequest("POST", "/api/security-costos/review", { movementKey, revisado });
      return res.json();
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["/api/security-costos"] }); },
  ...
  const handleToggleAll = async (filteredRowsSnapshot: SecurityRow[]) => {
    ...
    try {
      await Promise.all(
        filteredRowsSnapshot
          ...
          .map(r => apiRequest("POST", "/api/security-costos/review", { movementKey: r.movementKey, revisado: targetState }).then(res => res.json()))
      );
      queryClient.invalidateQueries({ queryKey: ["/api/security-costos"] });
    ...
    <div className="px-6 py-6 pb-8" style={{ width: "max-content", minWidth: "100%" }}>
      <div className="max-w-6xl space-y-6 mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight" data-testid="text-security-costos-title">
              Caja - Banco Security
            </h1>
            <p className="text-muted-foreground text-sm mt-0.5">
              Movimientos bancarios de la Cartola Banco Security
            </p>
          </div>
          {rows && rows.length > 0 && (
            <div className="ml-auto flex items-center gap-3">
              <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200 dark:bg-green-950 dark:text-green-300 dark:border-green-800" data-testid="badge-sec-revisados">
                <Check className="h-3 w-3 mr-1" />
                {statsResumen.revisados} revisados
              </Badge>
              <Badge variant="outline" className="bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-950 dark:text-amber-300 dark:border-amber-800" data-testid="badge-sec-pendientes">
              ...
              {filteredRows.length > 0 && (
                <Button
                  ...
                  size="sm"
                  className="gap-2"
                  data-testid="button-download-security"
                  onClick={() => {
                    const DOWNLOAD_COLS = [
                    ...
                    downloadAsXlsx(data, DOWNLOAD_COLS, "caja-security");
                  }}
                >
        ...
        <Card className="border-card-border">
          ...
          <CardContent>
            <div className="flex flex-wrap gap-4 items-end">
              ...
              <div className="space-y-1.5">
                <Label htmlFor="sec-filtro-centro" className="text-sm">Centro de Costos</Label>
                <Select value={filtroCentro} onValueChange={setFiltroCentro}>
                  <SelectTrigger className="w-56" data-testid="select-sec-filtro-centro">
              ...
              <div className="space-y-1.5">
                <Label htmlFor="sec-filtro-estado" className="text-sm">Estado</Label>
                <Select value={filtroEstado} onValueChange={setFiltroEstado}>
                  <SelectTrigger className="w-44" data-testid="select-sec-filtro-estado">
                    <SelectValue placeholder="Todos" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="todos">Todos</SelectItem>
              ...
              <div className="space-y-1.5">
                <Label htmlFor="sec-filtro-tipo" className="text-sm">Tipo transacción</Label>
                <Select value={filtroTipo} onValueChange={setFiltroTipo}>
                  <SelectTrigger className="w-44" data-testid="select-sec-filtro-tipo">
                    <SelectValue placeholder="Todos" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="todos">Todos</SelectItem>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
            <Card className="border-card-border">
              <CardContent className="pt-5 pb-4">
                ...
                <p className={`text-2xl font-bold ${(rows && rows.length > 0 ? (rows[0].saldoBanco ?? 0) : 0) >= 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`} data-testid="text-sec-saldo-actual">
            ...
            <Card className="border-card-border">
              <CardContent className="pt-5 pb-4">
                ...
                <p className="text-2xl font-bold text-green-600 dark:text-green-400" data-testid="text-sec-total-abonos">
            ...
            <Card className="border-card-border">
              <CardContent className="pt-5 pb-4">
                ...
                <p className="text-2xl font-bold text-red-600 dark:text-red-400" data-testid="text-sec-total-cargos">
            ...
            <Card className="border-card-border">
              <CardContent className="pt-5 pb-4">
                ...
                <p className="text-2xl font-bold text-orange-600 dark:text-orange-400" data-testid="text-sec-total-sin-nada">
        ...
        {user?.email === "natyjelen@gmail.com" && (
          <ImportCentrosCostosPanel
            apiEndpoint="/api/security/import-centros-costos"
            label="Security"
            columnDescription="FECHA, OBS, CENTROS DE COSTO, FACTURA/BOLETA, GIRO, DEPOSITOS, TOTAL"
          />
        )}
      </div>
      ...
      <Card className="border-card-border w-full">
        <CardHeader className="flex flex-row items-center justify-between gap-2 space-y-0 pb-4">
          <CardTitle className="text-base font-semibold" data-testid="text-sec-tabla-movimientos">
            Movimientos
          </CardTitle>
          {filteredRows.length > 0 && (
            <div className="flex items-center gap-4">
              <div className="text-right">
                ...
                <p className={`text-lg font-bold tabular-nums ${(rows && rows.length > 0 ? (rows[0].saldoBanco ?? 0) : 0) < 0 ? "text-red-600 dark:text-red-400" : "text-green-700 dark:text-green-400"}`} data-testid="text-sec-total-visible">
                  {formatMonto(rows && rows.length > 0 ? (rows[0].saldoBanco ?? 0) : 0)}
                </p>
              </div>
              <span className="text-xs text-muted-foreground border-l pl-4" data-testid="text-sec-row-count">
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-xs text-muted-foreground/70 mt-1">
                Sube un archivo de Cartola Banco Security en el módulo correspondiente
              </p>
            </div>
          ) : filteredRows.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <Table>
                {(() => {
                  ...
                  const SortBtn = ({ field, label, className }: { field: string; label: string; className?: string }) => (
                    <TableHead className={`${thBase} ${className ?? ""}`}>
                      <button className="flex items-center gap-0.5 hover:text-foreground transition-colors" onClick={() => toggleSort(field)} data-testid={`sort-sec-${field}`}>
                      ...
                      <TableRow>
                        ...
                        <TableHead className={`${thBase} w-14 text-center !pr-8`}>
                          <Checkbox
                            ...
                            data-testid="checkbox-sec-review-all"
                ...
                <TableBody>
                  {sortedRows.map((row, i) => (
                    <TableRow key={i} data-testid={`row-sec-movimiento-${i}`} className={row.revisado ? "bg-green-50/50 dark:bg-green-950/20" : ""}>
                      <TableCell className="text-sm" data-testid={`text-sec-fecha-${i}`}>{row.fecha}</TableCell>
                      <TableCell data-testid={`cell-sec-fecha-cobro-${i}`}>
                        <FechaCobroCell movementKey={row.movementKey} fechaCobro={row.fechaCobro} fechaEmision={row.fecha} onSave={handleSaveFechaCobro} />
                      </TableCell>
                      <TableCell className="text-sm max-w-[200px]" data-testid={`text-sec-detalle-${i}`}>
                        <div className="truncate" title={row.detalle}>{row.detalle}</div>
                      </TableCell>
                      <TableCell data-testid={`text-sec-ndocumento-${i}`}>
                      ...
                      <TableCell className={`text-sm font-medium text-right ${row.monto < 0 ? "text-red-600 dark:text-red-400" : "text-green-600 dark:text-green-400"}`} data-testid={`text-sec-monto-${i}`}>
                        {formatMonto(row.monto)}
                      </TableCell>
                      <TableCell className={`text-sm font-semibold text-right ${(row.saldoBanco ?? 0) < 0 ? "text-red-600 dark:text-red-400" : "text-foreground"}`} data-testid={`text-sec-saldo-${i}`}>
                        {row.saldoBanco != null ? formatMonto(row.saldoBanco) : "—"}
                      </TableCell>
                      <TableCell data-testid={`cell-sec-centro-costos-${i}`}>
                        {row.centroCostos ? (
                          <div className="flex items-center gap-2">
                            ...
                            <Select value={row.centroCostos} onValueChange={(val) => handleCentroChange(row.detalle, val, row.movementKey)}>
                              <SelectTrigger className="h-7 w-7 p-0 border-none shadow-none [&>svg]:h-3 [&>svg]:w-3" data-testid={`btn-sec-change-centro-${i}`}>
                          ...
                          <Select value="" onValueChange={(val) => handleCentroChange(row.detalle, val, row.movementKey)}>
                            <SelectTrigger className="h-7 w-36 text-xs text-muted-foreground" data-testid={`select-sec-centro-${i}`}>
                      ...
                      <TableCell className="text-center" data-testid={`cell-sec-estado-${i}`}>
                        {row.revisado ? (
                      ...
                      <TableCell className="text-center !pr-8">
                        {row.revisado ? (
                          ...
                          <Checkbox
                            ...
                            data-testid={`checkbox-sec-review-${i}`}
                            className="data-[state=checked]:bg-green-600 data-[state=checked]:border-green-600"
                          />
                        )}
                      </TableCell>
