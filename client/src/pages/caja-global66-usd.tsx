...
import { Landmark, Calendar, Filter, Check, Download, Pencil, X, ChevronUp, ChevronDown, ChevronsUpDown } from "lucide-react";
...
function getColorClass(centro: string, monto: number): string {
  ...
  if (!tone) return monto >= 0 ? "bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200" : "bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200";
...
function formatMontoUsd(value: number): string {
  ...
  return value < 0 ? `-US$${formatted}` : `US$${formatted}`;
}
...
function FechaCobroCell({ movementKey, fechaCobro, fechaEmision, onSave }: {
  movementKey: string; fechaCobro: string; fechaEmision: string;
  ...
  if (editing) {
    return (
      <input type="date" autoFocus defaultValue={toInputDate(fechaCobro)}
        ...
        data-testid={`input-fecha-cobro-g66usd-${movementKey}`}
      />
    );
  }
  ...
  return (
    <div className="flex items-center gap-1 group">
      <button onClick={() => setEditing(true)}
        ...
        data-testid={`text-fecha-cobro-g66usd-${movementKey}`}
      >{fechaCobro}</button>
      {isModified && (
        <button onClick={() => onSave(movementKey, null)}
          className="opacity-0 group-hover:opacity-60 hover:opacity-100 transition-opacity"
          title="Restaurar a fecha de emisión"
          data-testid={`btn-reset-fecha-cobro-g66usd-${movementKey}`}
        >
...
function NDocCell({ movementKey, nDocumentos, nDocIsOverride, candidatos, allRows, onSave }: {
  movementKey: string; nDocumentos: string[]; nDocIsOverride: boolean;
  candidatos: Candidato[]; allRows: G66UsdRow[];
  ...
  const candidatoMap = useMemo(() => {
    ...
    for (const c of candidatos) { if (c.descripcion) m.set(c.nDocumento, c.descripcion); }
    return m;
  }, [candidatos]);
  ...
  const filtered = useMemo(() => {
    ...
    return candidatos.filter(c => c.nDocumento.toLowerCase().includes(q) || c.descripcion.toLowerCase().includes(q)).slice(0, 60);
  }, [candidatos, search]);
  ...
  const attemptSave = (newVal: string) => {
    ...
    const duplicate = allRows.find(r => r.movementKey !== movementKey && r.nDocumentos.includes(newVal));
    if (duplicate) {
      msgs.push(`El N° ${newVal} ya está asociado al movimiento del ${duplicate.fecha} — "${duplicate.detalle.slice(0, 50)}".`);
    }
    if (msgs.length > 0) { setPendingValue(newVal); setWarnings(msgs); }
    else confirmSave(newVal);
  };

  const confirmSave = (val: string) => {
    onSave(movementKey, [...nDocumentos, val]);
    setOpen(false); setSearch(""); setManualValue(""); setPendingValue(null); setWarnings([]);
  };
  ...
  const TIPO_LABEL: Record<string, string> = {
    "FACTURA ELECTRÓNICA": "Fact.", "BOLETA ELECTRÓNICA": "Bol.", "FACTURA COMPRA": "Compra",
  };

  return (
    <Popover open={open} onOpenChange={(o) => { setOpen(o); if (!o) { setPendingValue(null); setWarnings([]); setSearch(""); } }}>
      <PopoverTrigger asChild>
        <button className="flex items-center flex-wrap gap-1 group focus:outline-none" data-testid={`btn-ndoc-g66usd-${movementKey}`}>
                ...
                return (
                  <Tooltip key={doc}>
                    <TooltipTrigger asChild>
                      <span>
                        <Badge variant={nDocIsOverride ? "secondary" : "outline"}
                          className={`text-xs font-mono cursor-pointer transition-colors pr-1 ${nDocIsOverride ? "bg-primary/15 text-primary border-0 hover:bg-primary/25" : "text-muted-foreground hover:text-foreground hover:border-foreground/40"}`}
                          data-testid={`badge-ndoc-g66usd-${doc}`}
                        >
                          #{doc}
                          {nDocIsOverride && (
                            <span role="button" onClick={(e) => handleRemoveOne(doc, e)} className="ml-1 opacity-50 hover:opacity-100 transition-opacity">
                    ...
                    {clientName && <TooltipContent side="top" className="max-w-[220px] text-center">{clientName}</TooltipContent>}
                  </Tooltip>
                );
              })}
            </TooltipProvider>
      ...
      <PopoverContent className="w-80 p-0" align="start">
        {warnings.length > 0 ? (
          <div className="p-3 space-y-3">
            ...
            <p className="text-xs text-muted-foreground">¿Deseas guardar igualmente el N° <span className="font-mono font-semibold text-foreground">#{pendingValue}</span>?</p>
            <div className="flex gap-2">
              <Button size="sm" className="h-7 text-xs flex-1" onClick={() => confirmSave(pendingValue!)} data-testid="btn-confirm-ndoc-g66usd">Confirmar</Button>
              <Button size="sm" variant="outline" className="h-7 text-xs flex-1" onClick={cancelPending} data-testid="btn-cancel-ndoc-g66usd">Cancelar</Button>
            </div>
          </div>
        ) : (
          <>
            ...
            <div className="p-2 border-b">
              <Input placeholder="Buscar N° documento o cliente..." value={search} onChange={(e) => setSearch(e.target.value)}
                className="h-8 text-xs" autoFocus data-testid="input-search-ndoc-g66usd"
              />
            </div>
            <ScrollArea className="h-44">
              <div className="py-1">
                {filtered.length === 0 ? (
                  ...
                  filtered.map((c) => (
                    <button key={`${c.tipo}|${c.nDocumento}`} onClick={() => attemptSave(c.nDocumento)} disabled={nDocumentos.includes(c.nDocumento)}
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        ...
                        {c.monto > 0 && <p className="text-[11px] text-muted-foreground leading-tight">US${Math.round(c.monto).toLocaleString("en-US")}{c.fecha ? ` · ${c.fecha}` : ""}</p>}
                      </div>
                    </button>
                  ))
                )}
            ...
            <div className="border-t p-2 space-y-2">
              ...
              <div className="flex gap-1.5">
                <Input value={manualValue} onChange={(e) => setManualValue(e.target.value)} placeholder="Ej: 12345"
                  className="h-7 text-xs" onKeyDown={(e) => e.key === "Enter" && attemptSave(manualValue.trim())} data-testid="input-manual-ndoc-g66usd"
                />
                <Button size="sm" className="h-7 text-xs px-2" onClick={() => attemptSave(manualValue.trim())} disabled={!manualValue.trim()} data-testid="btn-save-ndoc-g66usd">Guardar</Button>
              </div>
              {nDocIsOverride && (
                <Button variant="ghost" size="sm" className="h-7 text-xs text-muted-foreground w-full hover:text-destructive" onClick={handleClearAll} data-testid="btn-clear-ndoc-g66usd">
...
const API_PREFIX = "/api/global66-usd-costos";

export default function CajaGlobal66UsdPage() {
  ...
  const { data: rows, isLoading } = useQuery<G66UsdRow[]>({ queryKey: [API_PREFIX] });
  const { data: candidatos = [] } = useQuery<Candidato[]>({ queryKey: [`${API_PREFIX}/candidatos-ndoc`], staleTime: 5 * 60 * 1000 });

  const saveNDocMutation = useMutation({
    mutationFn: async ({ movementKey, nDocumentos }: { movementKey: string; nDocumentos: string[] }) => {
      const res = await apiRequest("POST", `${API_PREFIX}/save-ndocumento`, { movementKey, nDocumentos });
      return res.json();
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: [API_PREFIX] }); },
  ...
  const saveFechaCobroMutation = useMutation({
    mutationFn: async ({ movementKey, fechaCobro }: { movementKey: string; fechaCobro: string | null }) => {
      const res = await apiRequest("POST", `${API_PREFIX}/save-fecha-cobro`, { movementKey, fechaCobro });
      return res.json();
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: [API_PREFIX] }); },
  ...
  const saveCCMutation = useMutation({
    mutationFn: async ({ movementKey, centroCostos }: { movementKey: string; centroCostos: string }) => {
      const res = await apiRequest("POST", `${API_PREFIX}/save-cc`, { movementKey, centroCostos });
      return res.json();
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: [API_PREFIX] }); },
  ...
  const reviewMutation = useMutation({
    mutationFn: async ({ movementKey, revisado }: { movementKey: string; revisado: boolean }) => {
      const res = await apiRequest("POST", `${API_PREFIX}/review`, { movementKey, revisado });
      return res.json();
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: [API_PREFIX] }); },
  ...
  const handleToggleAll = async (filteredRowsSnapshot: G66UsdRow[]) => {
    ...
    try {
      await Promise.all(
        filteredRowsSnapshot
          ...
          .map(r => apiRequest("POST", `${API_PREFIX}/review`, { movementKey: r.movementKey, revisado: targetState }).then(res => res.json()))
      );
  ...
  const sortedRows = useMemo(() => {
    ...
    return [...filteredRows].sort((a, b) => {
      ...
      else if (sortField === "debito") cmp = a.debito - b.debito;
      else if (sortField === "abono") cmp = a.abono - b.abono;
  ...
  const fmtUsd = (v: number) => `US$${Math.abs(v).toLocaleString("en-US", { minimumFractionDigits: 0, maximumFractionDigits: 0 })}`;
    ...
    <div className="px-6 py-6 pb-8" style={{ width: "max-content", minWidth: "100%" }}>
      <div className="max-w-6xl space-y-6 mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight" data-testid="text-g66usd-costos-title">
              Caja - Global66 USD
            </h1>
            <p className="text-muted-foreground text-sm mt-0.5">
              Movimientos de la cuenta Global66 en dólares estadounidenses
            </p>
          </div>
          {rows && rows.length > 0 && (
            <div className="ml-auto flex items-center gap-3">
              <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200 dark:bg-green-950 dark:text-green-300 dark:border-green-800" data-testid="badge-g66usd-revisados">
                <Check className="h-3 w-3 mr-1" />{statsResumen.revisados} revisados
              </Badge>
              <Badge variant="outline" className="bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-950 dark:text-amber-300 dark:border-amber-800" data-testid="badge-g66usd-pendientes">
                {statsResumen.pendientes} pendientes
              </Badge>
              {filteredRows.length > 0 && (
                <Button variant="outline" size="sm" className="gap-2" data-testid="button-download-g66usd"
                  onClick={() => {
                    const COLS = [
                      ...
                      { key: "debito", label: "Débito (USD)" },
                      { key: "abono", label: "Abono (USD)" },
                      { key: "saldoBanco", label: "Saldo (USD)" },
                    ...
                    const data = filteredRows.map(r => ({
                      ...
                      tipoTransaccion: r.monto >= 0 ? "Abono" : "Débito",
                      debito: r.debito, abono: r.abono, saldoBanco: r.saldoBanco ?? "",
                      centroCostos: r.centroCostos ?? "", estado: r.revisado ? "Revisado" : "Pendiente",
                    }));
                    downloadAsXlsx(data, COLS, "caja-global66-usd");
                  }}
                >
          ...
          <CardContent>
            <div className="flex flex-wrap gap-4 items-end">
              <div className="space-y-1.5">
                ...
                <MonthFilter availableMonths={mesesDisponibles} selectedMonths={filtroMeses} onChange={setFiltroMeses} label="Todos los meses" />
              </div>
              <div className="space-y-1.5">
                <Label className="text-sm">Centro de Costos</Label>
                <Select value={filtroCentro} onValueChange={setFiltroCentro}>
                  <SelectTrigger className="w-56" data-testid="select-g66usd-filtro-centro"><SelectValue placeholder="Todos" /></SelectTrigger>
                  <SelectContent>
              ...
              <div className="space-y-1.5">
                ...
                <Select value={filtroEstado} onValueChange={setFiltroEstado}>
                  <SelectTrigger className="w-44" data-testid="select-g66usd-filtro-estado"><SelectValue placeholder="Todos" /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="todos">Todos</SelectItem>
              ...
              <div className="space-y-1.5">
                ...
                <Select value={filtroTipo} onValueChange={setFiltroTipo}>
                  <SelectTrigger className="w-44" data-testid="select-g66usd-filtro-tipo"><SelectValue placeholder="Todos" /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="todos">Todos</SelectItem>
                    <SelectItem value="ventas">Abonos</SelectItem>
                    <SelectItem value="costos">Débitos</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
            <Card className="border-card-border">
              <CardContent className="pt-5 pb-4">
                ...
                <p className={`text-2xl font-bold ${(rows && rows.length > 0 ? (rows[0].saldoBanco ?? 0) : 0) >= 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`} data-testid="text-g66usd-saldo-actual">
                  {formatMontoUsd(rows && rows.length > 0 ? (rows[0].saldoBanco ?? 0) : 0)}
                </p>
            ...
            <Card className="border-card-border">
              <CardContent className="pt-5 pb-4">
                ...
                <p className="text-2xl font-bold text-green-600 dark:text-green-400" data-testid="text-g66usd-total-abonos">{fmtUsd(totales.ventas)}</p>
                <p className="text-xs text-muted-foreground mt-1">Suma de montos positivos</p>
              </CardContent>
            </Card>
            <Card className="border-card-border">
              <CardContent className="pt-5 pb-4">
                <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-1">Total Débitos</p>
                <p className="text-2xl font-bold text-red-600 dark:text-red-400" data-testid="text-g66usd-total-debitos">{fmtUsd(Math.abs(totales.costos))}</p>
                <p className="text-xs text-muted-foreground mt-1">Suma de montos negativos</p>
              </CardContent>
            </Card>
            <Card className="border-card-border">
              <CardContent className="pt-5 pb-4">
                <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-1">Débitos sin "C-Nada"</p>
                <p className="text-2xl font-bold text-orange-600 dark:text-orange-400" data-testid="text-g66usd-total-sin-nada">{fmtUsd(Math.abs(totales.costossinNada))}</p>
                <p className="text-xs text-muted-foreground mt-1">Débitos excluyendo C-NADA</p>
              </CardContent>
            </Card>
          </div>
        )}
      ...
      <Card className="border-card-border w-full">
        <CardHeader className="flex flex-row items-center justify-between gap-2 space-y-0 pb-4">
          <CardTitle className="text-base font-semibold" data-testid="text-g66usd-tabla-movimientos">Movimientos</CardTitle>
          {filteredRows.length > 0 && (
            <div className="flex items-center gap-4">
              <div className="text-right">
                <p className="text-xs text-muted-foreground">Saldo</p>
                <p className={`text-lg font-bold tabular-nums ${(rows && rows.length > 0 ? (rows[0].saldoBanco ?? 0) : 0) < 0 ? "text-red-600 dark:text-red-400" : "text-green-700 dark:text-green-400"}`} data-testid="text-g66usd-total-visible">
                  {formatMontoUsd(rows && rows.length > 0 ? (rows[0].saldoBanco ?? 0) : 0)}
                </p>
              </div>
              <span className="text-xs text-muted-foreground border-l pl-4" data-testid="text-g66usd-row-count">
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-sm text-muted-foreground">No hay datos de cuenta cargados</p>
              <p className="text-xs text-muted-foreground/70 mt-1">Sube un archivo PDF de Global66 USD en el módulo de Subir Archivos</p>
            </div>
          ) : filteredRows.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <Table>
                {(() => {
                  ...
                  const SortBtn = ({ field, label, className }: { field: string; label: string; className?: string }) => (
                    <TableHead className={`${thBase} ${className ?? ""}`}>
                      <button className="flex items-center gap-0.5 hover:text-foreground transition-colors" onClick={() => toggleSort(field)} data-testid={`sort-g66usd-${field}`}>
                  ...
                  return (
                    <TableHeader>
                      <TableRow>
                        ...
                        <SortBtn field="debito" label="Débito" className="text-right w-28" />
                        <SortBtn field="abono" label="Abono" className="text-right w-28" />
                        <SortBtn field="saldoBanco" label="Saldo" className="text-right w-28" />
                        ...
                        <TableHead className={`${thBase} w-14 text-center !pr-8`}>
                          <Checkbox
                            ...
                            data-testid="checkbox-g66usd-review-all"
                ...
                <TableBody>
                  {sortedRows.map((row, i) => (
                    <TableRow key={i} data-testid={`row-g66usd-movimiento-${i}`} className={row.revisado ? "bg-green-50/50 dark:bg-green-950/20" : ""}>
                      <TableCell className="text-sm" data-testid={`text-g66usd-fecha-${i}`}>{row.fecha}</TableCell>
                      <TableCell data-testid={`cell-g66usd-fecha-cobro-${i}`}>
                        <FechaCobroCell movementKey={row.movementKey} fechaCobro={row.fechaCobro} fechaEmision={row.fecha} onSave={handleSaveFechaCobro} />
                      </TableCell>
                      <TableCell className="text-sm max-w-[200px]" data-testid={`text-g66usd-detalle-${i}`}>
                        <div className="truncate" title={row.detalle}>{row.detalle}</div>
                      </TableCell>
                      <TableCell className="text-xs text-muted-foreground font-mono" data-testid={`text-g66usd-movimiento-${i}`}>{row.movimientoNro || "—"}</TableCell>
                      <TableCell data-testid={`text-g66usd-ndocumento-${i}`}>
                      ...
                      <TableCell className="text-sm font-medium text-right text-red-600 dark:text-red-400 tabular-nums" data-testid={`text-g66usd-debito-${i}`}>
                        {row.debito > 0 ? formatMontoUsd(-row.debito) : "—"}
                      </TableCell>
                      <TableCell className="text-sm font-medium text-right text-green-600 dark:text-green-400 tabular-nums" data-testid={`text-g66usd-abono-${i}`}>
                        {row.abono > 0 ? formatMontoUsd(row.abono) : "—"}
                      </TableCell>
                      <TableCell className={`text-sm font-semibold text-right ${(row.saldoBanco ?? 0) < 0 ? "text-red-600 dark:text-red-400" : "text-foreground"}`} data-testid={`text-g66usd-saldo-${i}`}>
                        {row.saldoBanco != null ? formatMontoUsd(row.saldoBanco) : "—"}
                      </TableCell>
                      <TableCell data-testid={`cell-g66usd-centro-costos-${i}`}>
                        {row.centroCostos ? (
                          <div className="flex items-center gap-2">
                            <Badge variant="secondary" className={`text-xs whitespace-nowrap ${getColorClass(row.centroCostos, row.monto)}`}>{row.centroCostos}</Badge>
                            <Select value={row.centroCostos} onValueChange={(val) => handleCentroChange(row.detalle, val, row.movementKey)}>
                              <SelectTrigger className="h-7 w-7 p-0 border-none shadow-none [&>svg]:h-3 [&>svg]:w-3" data-testid={`btn-g66usd-change-centro-${i}`}>
                          ...
                          <Select value="" onValueChange={(val) => handleCentroChange(row.detalle, val, row.movementKey)}>
                            <SelectTrigger className="h-7 w-36 text-xs text-muted-foreground" data-testid={`select-g66usd-centro-${i}`}>
                      ...
                      <TableCell className="text-center" data-testid={`cell-g66usd-estado-${i}`}>
                        {row.revisado ? (
                      ...
                      <TableCell className="text-center !pr-8">
                        {row.revisado ? (
                          <button type="button" onClick={() => setEditConfirmKey(row.movementKey)}
                            className="inline-flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground transition-colors"
                            data-testid={`btn-g66usd-edit-${i}`}
                          >
                            <Pencil className="h-3.5 w-3.5" />Editar
                          </button>
                        ) : (
                          <Checkbox checked={false} onCheckedChange={() => handleReviewToggle(row.movementKey, row.revisado)}
                            data-testid={`checkbox-g66usd-review-${i}`}
                            className="data-[state=checked]:bg-green-600 data-[state=checked]:border-green-600"
                          />
                        )}
                      </TableCell>
